...
<table>
  <tr>
    <th align="center">欢迎页</th>
    <th align="center">登录页</th>
    <th align="center">智能问诊</th>
  </tr>
  <tr>
    <td align="center"><img src="docs/assets/github/welcome.png" alt="铭铭欢迎页" width="100%"/></td>
    <td align="center"><img src="docs/assets/github/login.png" alt="登录与游客入口" width="100%"/></td>
    <td align="center"><img src="docs/assets/github/chat.png" alt="智能问诊对话页" width="100%"/></td>
  </tr>
  <tr>
    <th align="center">问诊记录</th>
    <th></th>
    <th></th>
  </tr>
  <tr>
    <td align="center"><img src="docs/assets/github/records.png" alt="问诊记录列表页" width="100%"/></td>
    <td></td>
    <td></td>
  </tr>
</table>

当前 Web 端已覆盖 **游客登录、会话管理、多轮问诊、检查术语解读、指南 RAG 参考展示、问诊记录列表与续聊、详情抽屉** 等核心体验；移动端、多模态与支付挂号仍在路线图中。
...
| 接入 | gateway + **Sentinel** | 统一入口、JWT、**问诊限流 + Feign 降级** | Sentinel ✅ |
| 业务 | auth | 游客 / 短信登录 | ✅ |
...
> **限流**：**Sentinel** 挂在 Gateway（问诊接口 QPS）+ Session Feign 降级（ai-agent 故障），详见 [sentinel-setup.md](docs/sentinel-setup.md)。  
...
| 指南 RAG | LangGraph + Milvus 检索 + 关键词重排 | DeepSeek + Milvus | Milvus | ✅ |
...
| Graph 路由 | **纯规则**：`routing/rules.py` + `routing/keywords/` + sticky；边不调 LLM | ✅ |
| 多轮对话 | PostgreSQL checkpointer；Java 不传 history | ✅ |
| LoRA 微调（心血管问诊 SFT，可选本地推理） | 📋 [docs/lora-finetune.md](docs/lora-finetune.md) |
| 结构化输出 | `urgency` / `explanation` / `advice` / `disclaimer` / `guideReferences` | ✅ |
| 指南参考展示 | RAG 命中时在免责声明下方展示中文指南名；持久化至 MySQL / Redis | ✅ |
...
| 问诊记录 Worker | `cardiology-record` 无 HTTP；formal 空闲会话自动生成问诊记录与短标题，查询接口由 session 提供 | ✅ |
...
| 熔断限流 | **Sentinel**：Gateway 问诊/Auth 限流 + Feign→ai-agent 降级（[sentinel-setup.md](docs/sentinel-setup.md)） | ✅ |
| 分布式事务 | **Seata TCC**：Try 预占 → Confirm 提交 / Cancel 回滚（支付 + 挂号任务） | 📋 |
| 对象存储 | **阿里云 OSS**：多模态原文件、报告 PDF、预签名直传 | 📋 |
| 聚合支付 | **payment** 服务：微信 / 支付宝下单、回调、对账 | 📋 |
| 指南 RAG | Milvus 向量库检索心血管指南 PDF，重排后注入 LLM；前端展示参考指南名 | ✅ |
...
- **Milvus**：`services/ai-agent/guide/` 下 PDF 入库；`clinical_dispatch_node` 对 `symptom/lab/medication/history` 检索 top-K 片段注入 LLM
- **前端**：assistant 消息免责声明下方展示 `参考指南：{中文名}`；formal 存 `chat_message.guide_references`，guest 存 Redis JSON
- **本地验证**：`cd services/ai-agent && poetry run python scripts/test_rag_retrieval.py "高血压要做什么检查" --dispatch`
...
| 记录 Worker | [`services/cardiology-cloud/cardiology-record`](services/cardiology-cloud/cardiology-record/) | 无 HTTP；formal 会话生命周期 Job + 问诊总结 Job；记录查询走 session | [Nacos 配置](services/cardiology-cloud/nacos-config/cardiology-record-server.yaml) |
...
├── README.md
├── docker-compose.yaml              # 本地 MySQL / Redis / PostgreSQL / RabbitMQ / Nacos
...
└── services/
    ...
    │   ├── cardiology-record/       # 问诊记录 Worker（无 HTTP）· 生命周期 ✅ · 摘要 ✅
...
JDK 17 · Maven 3.9+ · Node.js 20+ · Yarn · Python 3.13+ · Poetry · Docker · MySQL 8 · Redis · RabbitMQ · Nacos · Milvus

### 0. 启动中间件

```bash
# 仓库根目录：启动中间件（含 Milvus、Nacos、Sentinel）
...
启动 MySQL（`3306`）、Redis（`6379`）、PostgreSQL（`5432`）、RabbitMQ（`5672` / 管理台 `15672`）、Nacos（`8080` / `8848`）、Milvus（`19530`）、Sentinel Dashboard（`8858`）。  
镜像默认走 **daocloud 镜像站**，国内 ECS / 本机均可拉取。  
`docker/mysql/init/` 在 **MySQL 首次初始化** 时建库建表；旧库补跑 `docker/mysql/migrations/`。

业务配置在 `application.yml` / `application-local.yml`；Nacos **仅服务发现**。Sentinel 限流规则见 gateway `classpath:sentinel-gateway-flow-rules.json`。
...
生产环境使用 [`docker-compose.prod.yaml`](docker-compose.prod.yaml)（**当前版本：`v1.0.0`**）：

```bash
cp deploy/.env.example deploy/.env   # MYSQL_*、POSTGRES_*、JWT、DEEPSEEK、ZHIPU_API_KEY 等
chmod +x deploy/deploy.sh scripts/pack-deploy.sh
./deploy/deploy.sh up -d --build
```

GitHub 拉不动时：`./scripts/pack-deploy.sh` → `scp cardiology-deploy.tgz` 到服务器解压后同样执行 `deploy.sh`。

浏览器访问 `http://<服务器IP>/`（Nginx 静态页 + `/api` 反代 gateway）。  
**Flutter App** 配置 `API_BASE_URL=https://<域名>/api`（或开发环境 `http://<IP>:30000`），与 Web **共用 JWT 与接口**。  
HTTPS、Sentinel、**指南 RAG 入库**、Postgres 密码、503 排查、磁盘清理见 [deploy/README.md](deploy/README.md)。
...
| 阶段 | 内容 |
|------|------|
| **已完成** | 铭铭问诊 MVP、PostgreSQL 多轮 checkpoint、消息落库、游客/正式分流、Gateway、JWT、会话管理、问诊记录、**Sentinel 限流降级**、Docker 全栈 |
| **进行中** | **LoRA 微调**（数据 → 训练 → 接入 LLMFactory A/B）、Web/Flutter 体验 |
| **规划中** | 多模态 Qwen、Seata/OSS、支付挂号、SSE 流式 |
