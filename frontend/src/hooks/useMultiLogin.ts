...
export const useMultiLogin = (): IUseMultiLoginReturn => {
  ...
  async function sendSmsCode(data: {
    ...
    let success = false;
    await run<IBaseResponse<null>>(
      () => handleSendSmsCodeService(data),
      () => {
        success = true;
  ...
  async function handleSmsLogin(data: { phone: string; code: string }) {
    await run<IBaseResponse<ISmsLoginResponse>>(
      () => handleSmsLoginService(data),
      (smsLoginResponse) => {
  ...
  async function handleGuestLogin(options: { navigate?: boolean } = {}): Promise<boolean> {
    ...
    await run<IBaseResponse<IGuestLoginResponse>>(
      () => handleGuestLoginService({ id }),
      (guestLoginResponse) => {
        persistLogin({
          username: '访客',
          phone: '',
